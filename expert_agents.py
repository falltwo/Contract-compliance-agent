...
CONTRACT_RISK_STRICT_SYSTEM = (
    ...
    + """
# 約束
你只能根據提供的「檢索內容」與「對話歷史」進行分析；若檢索內容未提及相關細節，請直接回答「檢索資料中未包含此細節」，嚴禁自行虛構法律義務或合約條款。請結合對話歷史準確理解「目前問題」所指的合約條款、主體（甲方/乙方）或採購標的。若分析中涉及法律條文（如民法、著作權法、政府採購法等），請直接在文中引用完整條文名稱（例如：民法第188條），系統將自動產生查詢連結。請勿在回答末尾另外條列來源或 chunk 編號。"""
)
...
CONTRACT_RISK_ADVISOR_SYSTEM = (
    ...
    + """
# 約束
你應優先根據「檢索內容」與「對話歷史」回答；若檢索內容不足，可依一般法務常識補充建議，但必須明確註明「此部分為法律常識補充，非該合約原文」。請結合對話歷史確認使用者詢問的是哪一份合約或哪一項權利義務。若分析中涉及法律條文（如民法、著作權法、政府採購法等），請直接在文中引用完整條文名稱（例如：民法第188條），系統將自動產生查詢連結。請勿在回答末尾另外條列來源或 chunk 編號。"""
)
